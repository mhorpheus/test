...
PagerDuty administrators for X-Force IR teams are:
...
* Initial setup was built by Adam Smutnicki (adam.smutnicki@pl.ibm.com) from X-Force IR EMEA - shall you have any questions reach out to him. 
...
It was not possible to transfer Hotline numbers between operators, so to integrate them under one of the providers X-Force IR would need to change several hotline numbers. X-Force IR decided it this is not acceptable scenario and decided to use PagerDuty (PD) for integration. All hotline numbers from RACC, Telstra and BT are pointing at single phone number offered by PD, so all incoming calls are redirected to PD. Then, every PD incoming call is redirected to a currently on-call person. 

X-Force IR PD hotline number is +44 808 178 1527. One can simply call this number and reach out directly PD system. X-Force IR has not advertised this number as official X-Force IR Hotline number using it only for integrations.
...
"Welcome. This is IBM X-Force IR Hotline where you can declare emergency security incident and ask for X-Force IR assistance. Make sure you have a valid client name and you are a designated contact eligible to engage X-Force IR. Please hold on while connecting to our Incident Response consultant. Thank you for waiting."

To provide fail-over mechanism as described in the [Hotline Operations](https://github.ibm.com/XFIR/DFIR-wiki/blob/master/DFIR-Hotline.md#Hotline-Operations), two on-call schedules are configured to represent primary and secondary on-call plan. When a new call arrives to PD, it redirects a call to a primary on-call person according to "[X-Force IR - Follow the sun (L1 - main)](https://ibm.pagerduty.com/schedules#PAL8VJX)" schedule (often referenced by team members as L1). If person is not available or do not pick up a call within 60 seconds, the call is redirected to a secondary on-call person according to "[X-Force IR - Follow the sun (L2 - overflow)](https://ibm.pagerduty.com/schedules#PB084QZ)" schedule (often referenced by team members as L2). If this person is not available or do not pick up a call for 60 seconds, PD will offer calling person an option to leave a Voice Message. Please see section [Handling Voice Messages](#Handling-Voice-Messages) for details on what happens when Voice Message is left by a person calling Hotline.
...
![Identifying caller number](https://github.ibm.com/XFIR/DFIR-wiki/blob/master/DFIR/PagerDuty-identify-caller.png)
...
PD has many functions supporting handling and escalating incidents. Due to sensitivity of data processed by X-Force IR as well as dedicated Resilient instance, PD is used only to handle Hotline calls. 
...
![PagerDuty Schedule view](https://github.ibm.com/XFIR/DFIR-wiki/blob/master/DFIR/PagerDuty_schedule_view.PNG)
...
Due to using a single set up for global X-Force IR team and lack granular enough rights configuration, umber of people who can schedule an override has been limited to ensure some level of protection against mistakes. Currently, overrides can be set up by anyone having a 'Manager' or 'Responder' role - to find necessary contact check out PD X-Force IR Admins list in the [Introduction](#Introduction) section.
...
4. Work with your manager, local IR Functional Lead and your regional Pager Duty admin to be added to on-call schedules, which includes:
	- adding person to a Global X-Force IR team in PD,
...
All timestamps in PD will be displayed according to the timezone set up in your profile, so it should be set up correctly. 
...
![Initial configuration of PagerDuty](https://github.ibm.com/XFIR/DFIR-wiki/blob/master/DFIR/PagerDuty-initial-config1.png)
...
![Finding VoiceMail messages in PagerDuty](https://github.ibm.com/XFIR/DFIR-wiki/blob/master/DFIR/PagerDuty-voicemail1.png)
4. Click on "Listen to Recording".
![Listening to VoiceMail messages in PagerDuty](https://github.ibm.com/XFIR/DFIR-wiki/blob/master/DFIR/PagerDuty-voicemail2.png)
...
![Exporting on-call calendar feed in PagerDuty](https://github.ibm.com/XFIR/DFIR-wiki/blob/master/DFIR/PagerDuty-calendar-feed.png)
...
![Identifying current on-call person](https://github.ibm.com/XFIR/DFIR-wiki/blob/master/DFIR/PagerDuty-who-is-on-call.png)
