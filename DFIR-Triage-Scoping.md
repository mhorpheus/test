...
XFIR is contractually obliged to organize a Triage and Scoping Call and meet defined SLA. The value of SLA depends on the type of the subscription. Default values are presented below (but there are exceptions documented in Resilient Client ticket):
...
When scheduling a Triage and Scoping call, it is recommended to provide at least 30 minutes between the hotline call and the Triage call to allow all participants to prepare.

A Triage and Scoping Call should be explicitly set up by XFIR Consultants, to ensure that XFIR owns and holds full control of the Triage. For this reason:
...
Joining ongoing calls pose a significant risk, that XFIR would not be driving the call, but will just be one of many parties involved and important topics related to security incident may be lost in discussions among big group of participants. 
...
3. initiate a Call:
	...
	- introduce XFIR members present on the call, clearly marking who would be leading the call.
...
Roles of participants from XFIR during Triage and Scoping Call can be defines as following:
...
1. The person who received a trigger for a Triage and Scoping call is capable of running a call on his/her own being a Senior Consultant as well as has necessary availability. A trigger can be a Hotline call or any other contact requesting XFIR assistance.
...
Triage and Scoping Call Summary Note and Mail is a an evidence on what was discussed during a call. It is a crucial outcome from the beginning of our engagement and should be prepared with necessary care. The role of the Note is to:
...
Please use below Triage and Scoping Call Summary Mail template and do not compose a note yourself. Template represents Phases of the Call documented above, thus should be easy to populate. Please make sure to fill in all fields and remove comments and unrelated statements.
...
`<In case non-sever incident when you can estimate effort, please use following statement. Do not use it for big incidents, with an "open bucket". >`
Based on information document above, XFIR initial estimation of number of hours necessary to deliver tasks listed above is `<XYZ>`. This number may change with any change in the scope of XFIR work. Any change in number of  estimated hours will be consulted to obtain your approval.
...
`<This is a final statement, make sure it is included. For Ad-hocs replace requirements to ACK by Incident Declarer to ACK by anyone from recipients.>`
IMPORTANT: To proceed further and engage XFIR for this incident, one of your representatives designated as "Incident Responder" in our database, MUST reply to this mail acknowledging content of the Triage and Scoping Call Note as well as approving subscription hours usage. If you disagree with any of the points in the mail or it anything was misunderstood, please let us know, so we can work on clarification.
`------------ END OF A TEMPLATE ------------ `

Important points:
- do not used XX.YY.ZZZZ date format, as it can be confusing due to American and European notation with day and month values swaped in places,
...
## Claiming
XFIR service is build in such a way, that anyone can request assistance, mainly by calling Hotline. Triage is offered free of charge irrespective of having subscription or not. It is not possible to provide claiming for all Triage participants. Only Triage Call Lead should:
