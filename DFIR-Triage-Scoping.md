...
# X-Force IR Triage and Scoping Call
...
## Introduction
The purpose of this page is to document the X-Force IR Triage and Scoping Call procedure. The following documentation is a guideline for X-Force IR Triage and Scoping Call procedures and is a living document. If there is something you want to see documented here or explained with further detail please contact your geographic Functional IR Lead (listed on the [Homepage](Home.md)).

A dedicated Triage and Scoping Call must be conducted for each assistance request received by X-Force IR. The intention of such a call is to:
...
- a call to the X-Force IR Hotline,
- direct contact from a Client to an X-Force IR Consultant they have a relationship with,
...
A Triage and Scoping Call should be explicitly set up by X-Force IR Consultants, to ensure that X-Force IR owns and holds full control of the Triage. This is  related to both participants and discussion flow. For this reason, wherever possible:
...
Joining ongoing calls poses a significant risk that X-Force IR would not be driving the call, and will just be one of many parties involved. Important topics related to the current security incident may be lost in discussions among a big group of participants. Instead offer to set up dedicated a Triage and Scoping Call between X-Force IR, the Client, and any other key stakeholders that they wish to invite.

The person organizing a Triage and Scoping Call is responsible for:
1. ensuring that X-Force IR is represented by required people (see Section [X-Force IR Representatives](#X-Force-IR-Representatives)),
2. setting up a call by sending Webex invites to designated participants,
3. initiating the Call:
	- open the Webex meeting,
	- provide a quick summary of a current X-Force IR understanding of the case,
	- introduce X-Force IR members present on the call, clearly mentioning who is the Triage and Scoping Call Lead.
...
- **Junior** or **Newcomer** (optional) - Consultants observing the Triage and Scoping Call process for their own experience so that they can build expertise or learn the X-Force IR approach to Triage and Scoping Calls.
...
*Triage Call Lead* should not be mistaken with a Incident Response Lead for this particular case, as it is possible that who would lead this investigation from X-Force IR was not decided prior to the Triage and Scoping Call.

Only necessary people should be invited to a Triage and Scoping Call, to avoid over-representation by the X-Force IR team, which could overwhelm Client. 
...
It is important to fully document **every** Triage and Scoping Call in Resilient, regardless of the final decision made by the client on whether or not to engage X-Force IR. 
...
1. Introduction:
	1. Introduce X-Force IR participants,
	2. Ask the Client to introduce their team together with their roles,
	3. If X-Force IR was engaged over the Hotline, the on-call person provides a quick summary of the current X-Force IR understanding of the incident.
...
2. Discovery:
	...
	2. Open the floor for questions from X-Force IR Consultants so they can get better understanding of the incident.
3. Identification of Client's objectives and expectations: 
	- what is needed to perform a successful delivery,
	- what type of tasks has the Client asked X-Force IR for help with: 
		- full IR?
		- dead box forensics?
		- EDR deployment?
		- log analysis?
		...
		- threat intel?
	- what is expected in terms of X-Force IR availability: 
		- on-site or remote?
		- 8x5, 8x7 or 24x7?
4. Presentation of X-Force IR approach and solution:
	...
	- provide initial/containment recommendations so Client can start immediately working on these tasks while X-Force IR set up our internal working environment,
	-  if applicable, X-Force IR Consultants can propose changes to a Client approach and expectations, explaining what would be the benefits from a change,
	- review approach proposed by X-Force IR against Client expectations, maybe Client wants to update their expectations/objectives for X-Force IR - agree on next steps.
5. Closing:
	...
	- Explain that X-Force IR cannot start work (and that the retainer is not formally triggered) until an email, from an Incident Declarer, approving the "Engagement Initiation Email" is received by X-Force IR.
...
The "Engagement Initiation Email" is a record of what was discussed during the Triage and Scoping call. It is a crucial document for managing client expectations from the beginning of our engagement, and should be prepared with care. Its role is to:
 ...
 - **ensure a common understanding with the Client** - protect X-Force IR against "we agreed/understood this differently" situations with Clients, as we explicitly require the Client to approve the content of the "Engagement Initiation Email", which states what actions were agreed for both sides as well as an estimate of the billable hours required for X-Force IR to complete their tasks.

Every "Engagement Initiation Email" must contain the sections defined below. Each section should contain several **key** points. It is not necessary to document every single sentence, as it is not a transcript of the Call. Common sense is necessary from the author of the "Engagement Initiation Email", as we do not want to the Client to wait 12 hours for a summary to be prepared. A well-written "Engagement Initiation Email" will allow any member of the X-Force IR team (including those that did not attend the Triage and Scoping Call) to read it and understand the agreed approach, as well as what the client has agreed to provide and how many hours we have available to complete the initial tranche of work.

The sections are as following:
1. X-Force IR understanding of the incident
2. Expectations and goals for X-Force IR
	- what the Client wants us to do and what we agreed upon.
	- what the Client has agreed to do in order for X-Force IR to be able to start work (e.g. provision access to their EDR for three consultants)
3. Next steps proposed by X-Force IR
...
5. Estimated cost (in hours) of planned X-Force IR work.
...
**Mail subcject:** `<Client Name>` Engagement Initiation Email for X-Force IR case IR`<YY-NNNN>`  
**Mail body:**  
Dear `<Client Name>`,  
...
**Our X-Force IR understanding of the incident:**  
	 - `<first bullet point>`  
 	 - `<second bullet point>`

**Expectation and goals for X-Force IR:**  
	 - `<first bullet point>`  
 	 - `<second bullet point>`
...
Based on information documented above, the X-Force IR initial estimation of hours necessary to deliver tasks listed above is `<XYZ>`. This number may change with any change in the scope of X-Force IR work or based on investigation findings. Any change in number of estimated hours will be communicated to obtain your approval.
...
Any decision on whether or not engage X-Force IR should be documented in the Resilient. Depending on the Client answer, potential next steps are:
1. X-Force IR gets engaged:
	- start [IR Spin-Up Process](DFIR-SpinUp.md),
	- populate "Owner/Engagement Lead" and "Members" fields accordingly in Resilient.
2. X-Force IR is not engaged:
...
## Claiming Time
The X-Force IR service is built in such a way that anyone can request assistance, mainly by calling the Hotline. Triage and Scoping Calls are offered free of charge irrespective of having a retainer or not. It is not possible to provide claiming for all Triage participants. Only Triage Call Lead should:
